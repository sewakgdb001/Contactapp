...
(function ($) { // Hide scope, no $ conflict
    ...
    entry functionality. Settings for fields are maintain`ed in an instance object,
    ...
    function DateEntry() {
        ...
        this._defaults = {
            ...
            minDate: new Date(1753, 1 - 1, 01), // The earliest selectable date, or null for no limit
            maxDate: null, // The latest selectable date, or null for no limit
            spinnerImage: null,
            // The URL of the images to use for the date spinner
            // Seven images packed horizontally for normal, each button pressed, and disabled
            /////////Added by Sachin//////////////
            //in this case it is set to nuss to avoid screen flicker
            spinnerSize: [20, 20, 8], // The width and height of the spinner image,
            // and size of centre button for current date
            spinnerBigImage: null, // The URL of the images to use for the expanded date spinner
            // Seven images packed horizontally for normal, each button pressed, and disabled
    ...
    $.extend(DateEntry.prototype, {
        ...
        _doKeyDown: function (event) {
            ...
            switch (event.keyCode) {
                //<<<<<<<<<<<<<<<To do Navigation on Next feild code updated>>>>>>>>>>>>>>>//        
                //<<<<<<<<<<<<<<<Updated by Sachin on 04/02/2010>>>>>>>>>>>>>>>//        
                case 9: return true;
                case 27: return true;
                ...
                case 13: return true;// added by priyanka to allow enter key in date control to navigate to next control
            }
            return false;
        },
...
function chkDate(na, curdt) {
    if (curdt.length == 0)  // for checking date length
    {
        return;
    }
    ...
    while ((curdt.charAt(i) != '/') && (i < curdt.length)) {

        daystring += curdt.charAt(i);
        i++;
    ...
    while ((curdt.charAt(i) != '/') && (i < curdt.length)) {
        monthstring += curdt.charAt(i);
        i++;
    }
    i++;

    while ((curdt.charAt(i) != '/') && (i < curdt.length)) {
        yearstring += curdt.charAt(i);
        i++;
    }

    if (yearstring < 1754) {
        alert('Year Should not be Less than "1754"!!!');
        document.all.item(na).value = ""
        document.all.item(na).focus();
        return false;
    }

    if (yearstring > 9999) {
        alert('Year Should not be more than "9999" !!!');
        document.all.item(na).value = ""
        document.all.item(na).focus();
        return false;
    }
