
$("input[type=submit], button[type=submit]").closest("form").not('.reportform').submit(function() {
    if (this.beenSubmitted)
    {

        alert("Already submitted.");
        return false;
    }
    else
    {
        this.beenSubmitted = true;
    }
    });


    // $('.printbtntable').click(function(){
        
        
         
    //      //var canvas =document.getElementById( "boothGenderwiseverticalbarchart" );
    // //alert(forverticalgendertableid.toDataURL());
    // //alert(new1)
    // //var $a=$(this).closest("canvas")
    // //alert('hello');
    // //alert($(this).closest('.card-header').find("span").html());
    
    
    // //$tr= $(this).closest('.card-body');//.prev('div .card-body');
    
    // //alert($tr.find("canvas").text());
    
    //  //$tr= $(this).closest('div.card');
    
    //  //var data = $tr.find("starcmptablebodydashboard")[0];
    //  $tr = $(this).closest('.card-body');
    //  var data=$(this).closest('starcmptbl');
    //  alert(data);
    // heading=$tr.find(".card-header span").text();
    
    // var html  = '<html><head><title>'+heading+'</title></head>';
    //  html += '<body  style=" width: 100%; padding: 0; margin-top: 25px;"';
    //  html += ' onload="window.focus(); window.print(); window.close()">';
    //  //html += '<img src="' + data + '" /></body></html>';
    //  html += "<table>\"" + data.outerHTML + "\"</table></body></html>";
    // var printWindow = window.open('', 'to_print', 'height=600,width=800');
    
    // printWindow.document.open();
    // printWindow.document.write(html);
    // printWindow.focus();
    // printWindow.print();
    // printWindow.document.close();
    // printWindow.close();
    //   /*var win = window.open();
    //  win.document.write(new1);
    //  win.print();
    //  win.close(); */
    //  //win.location.reload();
    
    
    // //veticalgenderchart.print();
    
    //  });




$('.printgraph').click(function(){
    // alert('a');
     //var canvas =document.getElementById( "boothGenderwiseverticalbarchart" );
//alert(forverticalgendertableid.toDataURL());
//alert(new1)
//var $a=$(this).closest("canvas")
//alert('hello');
//alert($(this).closest('.card-header').find("span").html());


//$tr= $(this).closest('.card-body');//.prev('div .card-body');

//alert($tr.find("canvas").text());

$tr= $(this).closest('div.card');

var data = $tr.find("canvas")[0].toDataURL();
heading=$tr.find(".card-header span").text();

var html  = '<html><head><title>'+heading+'</title></head>';
 html += '<body  style=" width: 100%; padding: 0; margin-top: 25px;"';
 html += ' onload="window.focus(); window.print(); window.close()">';
 //html += '<img src="' + data + '" /></body></html>';
 html += "<img src=\"" + data + "\" onload=\"javascript:window.print();\"/></body></html>";
var printWindow = window.open('', 'to_print', 'height=600,width=800');

printWindow.document.open();
printWindow.document.write(html);
printWindow.focus();
//printWindow.print();
//printWindow.document.close();
//printWindow.close();
  /*var win = window.open();
 win.document.write(new1);
 win.print();
 win.close(); */
 //win.location.reload();


//veticalgenderchart.print();

 }); 

    function checkajaxcall() {                  //check ajax call
        if ($.active != 0) {
            alert("Please wait while processing your request.");
            return false;
        }
        return true;
    }

    

    function getJSDateTimeFromStringDatetime(strdatetime){
        var ls = strdatetime.split(" ")
        var ls1 = ls[0].split("/")
        var day = Number(ls1[0]);
        var month = Number(ls1[1]);
        var year = Number(ls1[2]);
    
        var ls2 = ls[1].split(":");
        var hours = Number(ls2[0]);
        var minutes = Number(ls2[1]);
        var AMPM = ls[2];
        if(AMPM == "PM" && hours<12) hours = hours+12;
        if(AMPM == "AM" && hours==12) hours = hours-12;
        return new Date(year, month, day, hours, minutes, 0, 0);
    }



$(document).ready(function(){
    $('[data-toggle="tooltip"]').tooltip();
});

$(document).ready(function(){
    $('.hasdate,.hastime,.hasdatetime').on("cut copy paste",function(e) {
        e.preventDefault();
     });
     $(".hasdate,.hastime,.hasdatetime").attr("autocomplete", "off");
     
    $('.hasdate').dateEntry({dateFormat:'dmy/',spinnerImage: null});
    //$('.hasdate').focus();
     
    $('.hastime').timeEntry({spinnerImage: null,tabToExit:true});
    $('.hasdatetime').datetimeEntry({datetimeFormat: 'D/O/Y H:M a',spinnerImage: null,tabToExit:true,useMouseWheel:false});


    // var keyCodes = [61, 107, 173, 109, 187, 189];

    // $(document).keydown(function(event) {
    // if (event.ctrlKey==true && (keyCodes.indexOf(event.which) != -1)) {
    //     //alert('disabling zooming');
    //     event.preventDefault();
    //     }
    // });

    // $(window).bind('mousewheel DOMMouseScroll', function (event) {
    //     if (event.ctrlKey == true) {
    //     alert('disabling zooming');
    //     event.preventDefault();
    //     }
    // });
// Every time a modal is shown, if it has an autofocus element, focus on it.
$('.modal').on('shown.bs.modal', function() {
    $(this).find('[autofocus]').focus();
  });
  //$('.textsl,.amountfield-sm').bind("cut copy paste",function(e) {
    $('.textsl,.amountfield-sm').bind("cut copy",function(e) {
                e.preventDefault();
             });

            });

function validEmail(v) {
    var r = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
    //var r = new RegExp("[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?");
    return (v.match(r) == null) ? false : true;
}




function validatePhone(txtPhone) {
    var filter = /[6-9]{1}[0-9]{9}/;
    if (filter.test(txtPhone)) {
        return false;
    }
    else {
        return true;
    }
};

function translationInOtherLang(mainfield, slfiled,obj) {
    //$(slfiled).val($(mainfield).val().trim());
    //alert($("#"+slfiled).val());
   
    
    var tblrow = $($(obj).closest('tr'));

    
    
    if (tblrow.find(mainfield).val().trim() == '')
        {tblrow.find(slfiled).val('');}
    else{
    $.ajax({
        url:'/translateInotherLang/',
        data:{'input':tblrow.find(mainfield).val().trim()},
        datatype:'json',
        success: function(data){
            tblrow.find(slfiled).val(data.output);
            //alert(data.output);



        }
    });
    }
};


    function autoFillNamesInGrid(mainfield, slfiled,obj) {
        //$(slfiled).val($(mainfield).val().trim());
        //alert($("#"+slfiled).val());
        var tblrow = $($(obj).closest('tr'));
        if (tblrow.find(mainfield).val().trim() == ''){
            tblrow.find(slfiled).val('');
        }
        else{
            $.ajax({
                url:'/autoFillNames/',
                data:{'input':tblrow.find(mainfield).val().trim()},
                datatype:'json',
                success: function(data){
                    tblrow.find(slfiled).val(data.output);
                    //alert(data.output);
                }
            });
        }
    };


function translationInOtherLangid(mainfield, slfiled,obj) {
    //$(slfiled).val($(mainfield).val().trim());
    //alert($("#"+slfiled).val());
    if ($(mainfield).val().trim() == '')
        {$(slfiled).val('');}
    else{
    $.ajax({
        url:'/translateInotherLang/',
        data:{'input':$(mainfield).val().trim()},
        datatype:'json',
        success: function(data){
            $(slfiled).val(data.output);
            //alert(data.output);



        }
    });
    }
};

    function autoFillNamesWithId(mainfield, slfiled,obj) {
        
        
        //$(slfiled).val($(mainfield).val().trim());
        //alert($("#"+slfiled).val());
        if ($(mainfield).val().trim() == ''){
            $(slfiled).val('');
        }
        else{
            $.ajax({
                url: '/autoFillNames/',
                data:{'input':$(mainfield).val().trim()},
                datatype:'json',
                success: function(data){
                    if (data.error){
                        errorAlert();
                    }
                    else{
                        $(slfiled).val(data.output);
                        //alert(data.output);
                    }
                }
            });
        }
    };





// $(document).ready(function(){
//     $('body').bind("cut copy paste",function(e) {
//        e.preventDefault();
//     });

//     $("body").on("contextmenu",function(e){
//         return false;
//     });


//  });

// $(document).keydown(function(event) {
//     if (event.keyCode == 9) {  //tab pressed
//         event.preventDefault(); // stops its action
//     }
// })
// document.onkeydown = keydown;

// function keydown(event) {
//     //alert(event);
//         var code;
//         var e;
//         if (document.all) {
//             if (!event) {
//                 var e = window.event;
//                 code = e.keyCode;
//                 //alert(code);
//             }
//         }
//         else if (event.which) {
//         code = event.which;
//         e = event;
//         //alert('as')
//         }
//         if (code == 9) {//Write some logic
//         }
//     }


// for Ctrl+S save

document.addEventListener("keydown", function(e) {
    if (e.keyCode == 83 && (navigator.platform.match("Mac") ? e.metaKey : e.ctrlKey))      {
      e.preventDefault();
      //your implementation or function calls
      $('#btnsave').focus();
                  $('#btnsave').click();

    }
  }, false);


  document.addEventListener("keydown", function(e) {
    if (e.keyCode == 65 && (navigator.platform.match("Mac") ? e.metaKey : e.altKey))      {
      e.preventDefault();
      //your implementation or function calls
      $('#addindex').focus();
      
                  $('#addindex').click();

    }
  }, false);



//   $(document).keydown(function(e) {
//     if (e.altKey && e.which==65) {
//         e.preventDefault();

//             //alert('ctrl A');
//             $('#addindex').click();
//         }
//     }); 


   



$(document).ready(function () {

    //$(".inputs").focus()
    //$(this).next().focus();
    $("input:file").focus(function() { $(this).next().focus(); } );


    //$('input[tabindex=-1]').focus(function() { alert('asd');$(this).next().focus(); } );
    

$("form").delegate('input,a,select,button,textarea,ui-autocomplete-input,.input-sm,.form-control','keydown',function (e) {//keydown,keypress
   // alert(e);     
        //     var self = $(this)
        //   , form = self.parents('form:eq(0)')
        //              , focusable
        //              , next
        //          ;

        //  var parent = self.parents('div.ui-dialog:eq(0),.popupbox3','div.modal:eq(0)');
         /* on keypress tab
          *  on last element
          *   by priyanka :) 
          * on 30/1/2016
          * */
        //  if(((e.keyCode ==9 && !e.shiftKey )) && $(this)[0]==$(':input:visible:enabled').last()[0])
        // {
        //     e.preventDefault();
        //   $(':input:visible:enabled', document).eq(1).focus(); // save button
        //   return false;
        // }
        // // added by kashish: not to navigate on entering backspace
        // if((e.keyCode==8) && ($(this).attr('readonly')))
        // {
        //     event.preventDefault();
        //     return false;
        // }

        // added by kashish: not to navigate on entering backspace
        
        // if ($('.progressbar').css('display') == 'none' ) {

            var self = $(this)
                    , form = self.parents('form:eq(0)')
                    , focusable
                    , next
                    ;

            var parent = self.parents('div.ui-dialog:eq(0),.popupbox3','div.modal:eq(0)'); // if parent other than form
            //alert(e);
            //alert(e.which);

            if (e.keyCode == 13 ||  (e.which == 9 && e.shiftKey!==true) ) { // || e.keyCode ==9

                /* to check if 'this' is a
                 button
                 */
                if ($(this).is(':button ') || $(this).is(':submit')) {
                    var id = $(this).attr('id');
                    var fireClick;
                    //alert(fireClick || id );
                    if ($(".rel" + id).length) { // to check for relative controls to fire button click { in grid interface }
                        $(".rel" + id).each(function () {
//                            if ($(this).val().trim()=="" || $(this).val().trim()) {
                                fireClick = 1;
                                return false;
//                            }
                        });
                    }
                    else if (fireClick || id == "filebutton" && e.keyCode == 13) 
                    {
                        $(this).closest('div').find(':file ').click();

                    }
                   
                    else if(e.keyCode == 13){
                        //alert('sdasd');

                        $(this).click();
                        return false;
                    }
                    if (fireClick || id == "btnsave" && e.keyCode == 13) {
                        //alert('sdasd');
                        $(this).click();
                        return false;
                    }

                }

                else if ($(this).is(':file ') && e.keyCode == 13 ){
                    
                    $(this).click();
                    // alert('sdasd');
                    return false;
                    
                }
                if (form.length) { // if parent is form
                    //alert('1')
                    if($(this).is('a'))
                         focusable = form.find('.nav-link,input,a,select,button,textarea,ui-autocomplete-input, hasdate, input-sm,hasdatetime').filter(':visible');
                         else
                    focusable = form.find('.nav-link,input,a,select,button,textarea,ui-autocomplete-input, hasdate,.input-sm,hasdatetime').filter(':visible:enabled');
//                     focusable = form.find('input,a,select,button,textarea,ui-autocomplete-input, hasDateEntry').filter(':visible:enabled');
                }
                else { // if parent is not form // to resolve navigation issue in case of dialog
                    //alert('2')

                    
                    if($(this).is('a'))
                         focusable = parent.find('input,a,select,button,textarea,ui-autocomplete-input, hasdate ,input-sm,hasdatetime').filter(':visible');
                         else
                    focusable = parent.find('input,a,select,button,textarea,ui-autocomplete-input, hasdate,input-sm,hasdatetime').filter(':visible:enabled');
                    
                }
                //var tabIndex = +$el.attr( "tabindex" );
                //tabIndex = isNaN( tabIndex ) ? -1 : tabIndex;
                //return $el.is( ":input, a[href], area[href], iframe" ) || tabIndex > -1;
                next = focusable.eq(focusable.index(this) + 1);
                var tabIndex = +next.attr( "tabindex" );
                //alert(tabIndex)
                tabIndex = isNaN( tabIndex ) ? 0 : tabIndex;
                //alert(typeof tabIndex)
                //alert(tabIndex)
                if (next.is(":disabled") ) { // if next control is disabled then finding next enabled control to focus on...
                    next = focusable.find('input,select,a,button,textarea,ui-autocomplete-input,input-sm,hasdatetime,hasdate:gt(' + focusable.index(this) + '):enabled:first');
                    
                }
                else if( tabIndex == -1){
                    next = focusable.find('input,select,a,button,textarea,ui-autocomplete-input,input-sm,hasdatetime,hasdate:gt(' + focusable.index(this) + '):enabled:first');

                }
                
                if (next.length) {
                    next.focus();
                }
                else {

                    if ($("#current").length > 0) {
                        //alert('a');

                        var a = $("#current").nextAll(':visible:first').find('a').click(); //  next tab link
                        
                        if (a.length > 0) { ////  next tab link exists or not
                            var tab = a.attr("title");

                            focusable = $('#' + tab + ":first").find('input,a,select,button,textarea,ui-autocomplete-input, hasdate,input-sm,hasdatetime').filter(':visible:enabled');
                            next = focusable.eq(focusable.index(this) + 1);

                            if (next.length) { // whether tab has any input control
                                next.focus();
                                return false;
                            }
                            else {  // if no input control then focus on save 

                                if ($('#addindex').length) {
                                    $("#addindex").focus();
                                }
                                 else if ($('#btnsave').length) {
                                    // do something
                                    $("#btnsave").focus();
                                  }
                                  else if ($('#close-btn').length) {
                                    $("#close-btn").focus();

                                  }
                                  else{}

                                //$('#addindex').focus();

                                return false;
                            }
                        }
                        else {

                            if (form.parent().length > 0) {

                                if ($('#addindex').length) {
                                    $("#addindex").focus();
                                }
                                 else if ($('#btnsave').length) {
                                    // do something
                                    $("#btnsave").focus();
                                  }
                                  else if ($('#close-btn').length) {
                                    $("#close-btn").focus();



                            }else{}

                                //$('#addindex').focus();

                            }
                            else {
                                if (parent.hasClass('ui-dialog')) { // to check if no next control in current tab  the focus on Done button
                                    parent.find(".diaDone").focus();
                                }
                                else {
                                    if ($('#addindex').length) {
                                        $("#addindex").focus();
                                    }
                                     else if ($('#btnsave').length) {
                                        // do something
                                        $("#btnsave").focus();
                                      }
                                      else if ($('#close-btn').length) {
                                        $("#close-btn").focus();


  
                            }
                            else{}

                                }
                            }
                        }
                    }
                    else if ($(".tabs-nav .active .nav-tabs").length > 0) {
                        var a = $(".tabs-nav .active .nav-tabs").nextAll(':visible:first').find('a').click();

                        if (a.length > 0) {
                            var tab = a.attr("href");

                            focusable = $(tab + ":first").find('input,a,select,button,textarea,ui-autocomplete-input, hasDate ,input-sm,hasdatetime').filter(':visible:enabled');
                            next = focusable.eq(focusable.index(this) + 1);
                            next.focus();
                        }
                        else {
                            if (form.parent().length > 0) {


                                if ($('#addindex').length) {
                                    $("#addindex").focus();
                                }
                                 else if ($('#btnsave').length) {
                                    // do something
                                    $("#btnsave").focus();
                                  }
                                  else if ($('#close-btn').length) {
                                    $("#close-btn").focus();



}else{}

                                
                            }
                            else {
                                if ($('#addindex').length) {
                                    $("#addindex").focus();
                                }
                                 else if ($('#btnsave').length) {
                                    // do something
                                    $("#btnsave").focus();
                                  }
                                  else if ($('#close-btn').length) {
                                    $("#close-btn").focus();



}else{}

                            }
                        }
                    }
                    else {
                        if (form.parent().length > 0) {
                            if ($('#addindex').length) {
                                $("#addindex").focus();
                            }
                             else if ($('#btnsave').length) {
                                // do something
                                $("#btnsave").focus();
                              }
                              else if ($('#close-btn').length) {
                                $("#close-btn").focus();



}else{}

                        }
                        else {
                            if ($('#addindex').length) {
                                $("#addindex").focus();
                            }
                             else if ($('#btnsave').length) {
                                // do something
                                $("#btnsave").focus();
                              }
                              else if ($('#close-btn').length) {
                                $("#close-btn").focus();



}else{}

                            //$('#addindex').focus();
                        }
                    }

                }

                return false;
            }
        //}
    });





$("div.modal").delegate('input,a,select,button,textarea,ui-autocomplete-input,.input-sm','keydown',function (e) {
    if(((e.keyCode ==9 && !e.shiftKey )) && $(this)[0]==$(':input:visible:enabled').last()[0])
    {
        e.preventDefault();
        $(':input:visible:enabled', $("div.modal")).eq(0).focus(); 
        return false;
    }
    });

    $(document).keydown(function(e)
    {
        if(e.keyCode==8 && !$(e.target).is("input,textarea"))
            return false;
    })

});


$('.numeric').keypress(function (event){
    return AllowOnlyDecimal(event, this); 
 });
 // added by kashish
 $('.onlynumeric').keypress(function (event){
    return AcceptOnlyNumeric(event, this); 
 });
 
 // added by Sahil Sehgal
//  $('.onlyalphabetic').keypress(function (event){
//     return AcceptOnlyAlphabetic(event, this); 
//  });
 
//  function AcceptOnlyAlphabetic(eventObj, obj) {
//     var keyCode
//     // Check For Browser Type
//     if (document.all) {
//         keyCode = eventObj.keyCode
//     }
//     else {
//         keyCode = eventObj.which
//     }
//     //ends here
//     var str = obj.value

//     if ((keyCode >=97 && keyCode <=122) || (keyCode >=65 && keyCode <=90))
//     {
//         return true;
//     }
//     return false;
// }

// $(".onlyalphabetic").keypress(function(event){
//     alert(1)
//     var inputValue = event.which;
//     // alert(inputValue);
//     // allow letters and whitespaces only.

//     if( !( (inputValue >= 65 && inputValue <= 90) | ( inputValue >= 97 && inputValue <= 122 ) ) && (inputValue != 32 && inputValue != 0 && inputValue != 46 && inputValue != 47)) { 
//         event.preventDefault(); 
//     }

// });


$('.onlyalphabetic').keypress(function (event){
    return AcceptOnlyAlphabetic(event, this); 
 });
 
 function AcceptOnlyAlphabetic(eventObj, obj) {
 var keyCode
 // Check For Browser Type
 if (document.all) {
     keyCode = eventObj.keyCode
 }
 else {
     keyCode = eventObj.which
 }
 //ends here
 var str = obj.value

 if (  (keyCode >= 65 && keyCode <= 90) || keyCode == 32 || (keyCode >= 97 && keyCode <= 122)  )
 {
     return true;
 }
 return false;
}


$('.onlyalphabeticwithoutspace').keypress(function (event){
    return AcceptOnlyAlphabeticWithoutSpace(event, this); 
 });
 
 function AcceptOnlyAlphabeticWithoutSpace(eventObj, obj) {
 var keyCode
 // Check For Browser Type
 if (document.all) {
     keyCode = eventObj.keyCode
 }
 else {
     keyCode = eventObj.which
 }
 //ends here
 var str = obj.value

 if (  (keyCode >= 65 && keyCode <= 90) ||  (keyCode >= 97 && keyCode <= 122)  )
 {
     return true;
 }
 return false;
}


    function numericTagIt(eventObj, obj) {
        var keyCode;
        if (document.all) {
            keyCode = eventObj.keyCode
        }
        else {
            keyCode = eventObj.which
        }
        var str = obj.value
        if ( (keyCode >= 48 && keyCode <= 57) || keyCode == 32 || keyCode == 44 ){
            return true;
        }
        return false;
    }


    function getName(strdata) {

        if (strdata == undefined){
            return '';
        }
        else if (strdata.trim() == ""){
            return '';
        }
        else{
            var userrole = strdata.trim();
            var ls = userrole.split("-");
            var roleid = ls[ls.length-1];
            var i;
            var role = '';
            for (i = 0; i < ls.length-1; i++) { 
                role += ls[i];
            }
            return role;
        }
    }

    function getIdFromString(strdata){
        if (strdata == undefined){
            return '0';
        }
        else if (strdata.trim()==""){
            return '0';
        }
        else{
            var ls=strdata.split('-');
            if (ls.length > 1){
                id=ls[ls.length-1];
                return id;
            }
           else{
            return '0';
           } 
        }
    };

    function getNameFromString(strdata){
        if (strdata == undefined){
            return '';
        }
        else if (strdata.trim() == ""){
            return '';
        }
        else{
            var userrole = strdata.trim();
            var ls = userrole.split("-");
            var roleid = ls[ls.length-1];
            var i;
            var role = '';
            for (i = 0; i < ls.length-1; i++) { 
                role += ls[i];
            }
            return role;
        }
    };


    function getAutoCompleteTxtWithoutId(strdata){
        if ( strdata == undefined | strdata == 'undefined' | strdata.trim()==""){
            return '';
        }
        else{
            return strdata.slice(0, strdata.lastIndexOf("-"));
        }
    }

    function errorAlert(){
        alert("Some Error Occured.. Please Contact With Site Operator!!");
    }

    function noPermAlert(){
        alert("You are not authorized.");
    }

    function converttointcal(no)
    {
        var formatno=0;
        if(no!="")
        {
            if(!isNaN(no))
            {
                formatno=parseInt(no);
            }
        }
        return formatno;
    };

    function convertDMYToYMDFormatDate(d1){

        //const dateString = '22/11/2017 10:53 AM';
//const changedDate = dateString.replace(/(..)\/(..)\/(....) (..):(..) (..)/, '$3-$2-$1 $4:$5 $6');
//var date = new Date(changedDate);
changedDate =d1.replace(/(..)\/(..)\/(....) (..):(..) (..)/, '$3-$2-$1 $4:$5 $6');

//var date = new Date(changedDate);
return changedDate
    }


function campareDateTime(strd1,strd2) {
    //alert(strd1,'stdate')
    //alert(strd2,'end')
    if (strd1=='' || strd2==''){
        return true;
    }
    else{
        var d1 =Date.parse(convertDMYToYMDFormatDate(strd1));
        var d2 = Date.parse(convertDMYToYMDFormatDate(strd2));
  
    //var d1 = Date.parse(strd1);//Date.parse("2012/11/01 03:04 PM");
    //var d2 = Date.parse(strd2);//Date.parse("2012/11/01 03:04 PM");
    if (d1<  d2) {
        return true;
    //alert ("Error! Date did not Match");
     }
     else{
        return false;

        // alert ("Error! Date did not Match!!!!!!!!");

     }
    }
}

    function datechecks(_date)
    {
        var smaller = firstdate.trim().split('/');
        var smallerdate = Date.parse(smaller.reverse());
        if (smallerdate >= largerdate)
        {
            return 0;
        }
        else
        {
            return 1;
        }
    };

    function dategreater(firstdate)
    {
        var smaller = firstdate.split('/');
        var smallerdate = Date.parse(smaller.reverse());
        var d = new Date();

        if (smallerdate >d)
        {
            return 0;
        }
        else
        {
            return 1;
        }
    };

    function checkDateLessThanOrequalToCurrDate(strdate){
        var strdate = strdate.split('/');
        var strdate = Date.parse(strdate.reverse());
        var d = new Date();
        if (strdate>d){
            return false;
        }
        else{
            return true;
        }
    }
    


    function datecheckequals(firstdate, seconddate)
    {
        var smaller = firstdate.trim().split('/');
        var larger = seconddate.trim().split('/');
        var smallerdate = Date.parse(smaller.reverse());
        var largerdate = Date.parse(larger.reverse());
        if (smallerdate==largerdate)
        {
            return 0;
        }
        else
        {
            return 1;
        }
    };
    
    function datecheckgreater(firstdate, seconddate)
    {
        var smaller = firstdate.trim().split('/');
        var larger = seconddate.trim().split('/');
        var smallerdate = Date.parse(smaller.reverse());
        var largerdate = Date.parse(larger.reverse());
        //alert(smallerdate);
        //alert(largerdate);
        if (smallerdate > largerdate)
        {
            return 0;
        }
        else
        {
            return 1;
        }
    };

   

    function datecheckgreaterequal(firstdate, seconddate)
    {
        var smaller = firstdate.trim().split(' ')[0].trim().split('/');
        var larger = seconddate.trim().split('/');
        var smallerdate = Date.parse(smaller.reverse());
        var largerdate = Date.parse(larger.reverse());

        if (smallerdate <= largerdate )
        {
            return 1;
        }
        else
        {
            return 0;
        }
    };

    function datecheckbetweenequal(firstdate, seconddate,thirddate)
    {
        //alert(firstdate.trim().split(' ')[0])
        var smaller = firstdate.trim().split(' ')[0].trim().split('/');
        var larger = seconddate.trim().split(' ')[0].trim().split('/');
        var thirddate = thirddate.trim().split('/');

        var thdate = Date.parse(thirddate.reverse());


        var smallerdate = Date.parse(smaller.reverse());
        var largerdate = Date.parse(larger.reverse());


        if (smallerdate<=thdate)
        {   if(largerdate >= thdate){
            return 1;
        }
        else{
            return 0;
        }
        }
        else{
            return 0;
        }
    };



     var specialKeys = new Array();
     specialKeys.push(8);  //Backspace
     specialKeys.push(9);  //Tab
     specialKeys.push(46); //Delete
     specialKeys.push(36); //Home
     specialKeys.push(35); //End
     specialKeys.push(37); //Left
     specialKeys.push(39); //Right
 
    function IsAlphaNumeric(e) {
        var keyCode = e.keyCode == 0 ? e.charCode : e.keyCode;
        var ret = ((keyCode >= 48 && keyCode <= 57) || (keyCode >= 65 && keyCode <= 90) || keyCode == 32 || (keyCode >= 97 && keyCode <= 122) || (specialKeys.indexOf(e.keyCode) != -1 && e.charCode != e.keyCode));
        // document.getElementById("error").style.display = ret ? "none" : "inline";
        return ret;
    };



    function forTagsTxt(e) {
        var keyCode = e.keyCode == 0 ? e.charCode : e.keyCode;
        var ret = ((keyCode >= 48 && keyCode <= 57) || (keyCode >= 65 && keyCode <= 90) || keyCode == 32 || keyCode == 45 || keyCode == 95 || (keyCode >= 97 && keyCode <= 122) || (specialKeys.indexOf(e.keyCode) != -1 && e.charCode != e.keyCode));
        // document.getElementById("error").style.display = ret ? "none" : "inline";
        return ret;
    };



    function restrictAmpersand(e) {
        var keyCode = e.keyCode == 0 ? e.charCode : e.keyCode;
        var ret = (keyCode != 38);
        // document.getElementById("error").style.display = ret ? "none" : "inline";
        return ret;
    };



    function OnlyCapitalLetters(e) {
        var keyCode = e.keyCode == 0 ? e.charCode : e.keyCode;
        var ret = ( (keyCode >= 65 && keyCode <= 90) );
        // document.getElementById("error").style.display = ret ? "none" : "inline";
        return ret;
    };

    function validateEmailId(_emailid){
        return _emailid.endsWith('@gmail.com') || _emailid.endsWith('@yahoo.com') || _emailid.endsWith('@hotmail.com') || _emailid.endsWith('@icloud.com');
    }

    function isNumberKey(evt, element) {
        var charCode = (evt.which) ? evt.which : event.keyCode
        if (charCode > 31 && (charCode < 48 || charCode > 57) && !(charCode == 46 || charCode == 8))
          return false;
        else {
          var len = $(element).val().length;
          var index = $(element).val().indexOf('.');
          if (index > 0 && charCode == 46) {
            return false;
          }
          if (index > 0) {
            var CharAfterdot = (len + 1) - index;
            if (CharAfterdot > 3) {
              return false;
            }
          }
      
        }
        return true;
      }
    
function checktime(starttime,endtime)
{
    var valid=true;
    var start_time=starttime;
    var end_time=endtime;

    //stat time
    var ampm= start_time.substr(-2);
    var clk = start_time.substr(0, 5);
    var start_time=clk+' '+ampm;
    
    //end time
    var ampm= end_time.substr(-2);
    var clk = end_time.substr(0, 5);
    var end_time=clk+' '+ampm;

    //convert both time into timestamp
    var stt = new Date("November 13, 2013 " + start_time);
    stt = stt.getTime();

    var endt = new Date("November 13, 2013 " + end_time);
    endt = endt.getTime();

    //by this you can see time stamp value in console via firebug
    console.log("Time1: "+ stt + " Time2: " + endt);

    if(stt >= endt) {
        //alert('Please enter Start time smaller than End Time.');
        //$('#indmeetingstartdate').val('');
        //$('#indmeetingstartdate').focus();
        valid=false;
    }
    

    return valid;

}


function downloadexceldt(tblid,xis,yis,pagetitle,filename,tophead,lstbld=true,permprint){
    //tblid  table id
    //xis, Scroll to X axis is True
    //yis, Scroll to Y axis is value in px to height of our table
    //title, title of excel file 
    //filename to save file with name 
    //lstbld is  show by last entry is bold or normal default bold showd
   // alert(permshow);
    //var pageheader=tophead+"\n"+title
    if(permprint == 'True'){
       list = [
            
        {
        'extend': 'excelHtml5',
        'customize': function(xlsx) {
            var sheet = xlsx.xl.worksheets['sheet1.xml'];
            //$('row', sheet).attr('s', '51');
            $('row:eq(1) c', sheet).attr('s', '2');
            if (lstbld==true)
            {
                $('row:last c',sheet).attr('s', '2');
            }
            //
            //color the background of the cells if found the value
            
        
        },
        
        'title':tophead ,// $("#memberall").text()+" "+ $("#memberdetail").text(),
        'messageTop': pagetitle,
        'text': 'Export excel',
        'className': 'exportExcel',

        'filename': filename,//'Membership Report',
        'exportOptions': {
            'columns': ':visible',
            'modifier': {
            'page': 'all'
            }
        },
        
        },
        {
            "extend": 'pdfHtml5',
            // "customize": function ( doc ) {
            //     doc.content.splice( 1, 0, {
            //         margin: [ 0, 0, 0, 12 ],
            //         alignment: 'center',
            //         image: 'data:image/png;base64,'
            //     } );
            // },
            "exportOptions": {
                
                "columns": ':visible',
                'modifier': {
                    'page': 'all'
                    }
            },
            // "orientation": 'landscape',
            // "pageSize": 'LEGAL',
            "header":true,
            "footer": true,
            //'title':tophead+"\n"+pagetitle ,// $("#memberall").text()+" "+ $("#memberdetail").text(),
            //'messageTop': title,
            //'duplicate':true,
            'text': 'Export PDF',
            //'className': 'exportExcel',
            'filename': filename,
            "customize": function (doc) {
                console.log("doc: ",doc)
                //Remove the title created by datatTables
                //doc.content.splice(0,1);
                //Create a date string that we use in the footer. Format is dd-mm-yyyy
               // var now = new Date();
               // var jsDate = now.getDate()+'-'+(now.getMonth()+1)+'-'+now.getFullYear();
                doc.pageMargins =[10,30,20,20]//[10, 10, 45, 20]; //[10,10,10,10];
                // Set the font size fot the entire document
                doc.defaultStyle.fontSize = 10;
                //doc.defaultStyle.alignment="center";
                doc.content[0].text=tophead+"\n"+pagetitle
              // doc.defaultStyle.alignment = 'center';
              if (doc.content[1].table.body[0].length > 6){
                  if (doc.content[1].table.body[0].length == 7)
                  {
                    doc.content[1].table.widths="*";
                  }
                  else{
                      doc.content[1].table.widths="auto";
                  }

                    
                doc.pageOrientation= "landscape";
                doc.pageSize='LEGAL';
                doc.styles.tableHeader.alignment="left";
                var dataobj=doc.content[1].table.body
                $.each(dataobj, function (index, value) {
                    //console.log("Index: ",index, value);
                    if (index > 0){
                        $.each(value, function (index1, value1) {
                            //console.log("Index1: ",index1.lastIndexOf, value1);
                            if (index1==0)
                            {
                                doc.content[1].table.body[index][0].alignment = 'center';
                            }

                                
                           
                        });
                        //doc.content[1].table.body[index].alignment = 'right';
                    }
                });
              }
              else{
                doc.content[1].table.widths="*";
                //doc.content[1].table.alignment="center";
                doc.pageOrientation= "portrait";
                doc.pageSize='A4';

                doc.styles.tableHeader.alignment="center";
                var dataobj1=doc.content[1].table.body
                $.each(dataobj1, function (index, value) {
                    //console.log("Index: ",index, value);
                    if (index > 0){
                        $.each(value, function (index1, value1) {
                            //console.log("Index1: ",index1.lastIndexOf, value1);
                            if (index1==0)
                            {
                                doc.content[1].table.body[index][0].alignment = 'left';
                            }
                            else if (index1 == 4){
                                doc.content[1].table.body[index][4].alignment = 'right';
                            }
                            else{
                               // doc.content[1].table.body[index][].alignment = 'right';
                                doc.content[1].table.body[index][index1].alignment = 'center';
                            }

                                
                           
                        });
                        //doc.content[1].table.body[index].alignment = 'right';
                    }
                });
              }
               
               doc.content[1].layout="Borders";
               doc.content[1].margin=20;
                // Set the fontsize for the table header
                doc.styles.tableHeader.fontSize = 10;
                //doc.styles.tableBodyEven.alignment="right"
                //doc.styles.tableBodyOdd.alignment="right"
                
               

                // Create a header object with 3 columns
                // Left side: Logo
                // Middle: brandname
                // Right side: A document title
                doc.styles.title = {
                    fontSize: '15',
                    alignment: 'center',
                    margin:10,
                  }   
                // doc['header']=(function() {
                //     return {
                //         columns: [
                            
                //             {
                                
                //                 alignment: 'center',
                //                 //italics: true,
                //                 text: tophead+"\n"+pagetitle,
                //                 fontSize: 14,
                //                 margin: [10,10]
                //             },
                //             {
                                
                //                 alignment: 'center',
                //                 fontSize: 14,
                //                 text: pagetitle
                //             }
                //         ],
                //         width:'*',
                //         margin: 20
                //     }
                // });
                doc['footer']=(function(page, pages) {
                    return {
                        columns: [
                            //'This is your left footer column',
                            {
                                // This is the right column
                                alignment: 'center',
                                text: ['page ', { text: page.toString() },  ' of ', { text: pages.toString() }]
                            }
                        ],
                        margin: [20, 0]
                    }
                });
            },

            
            
            },
        ]
    }
    else {
        list = []
    }

    $(tblid).DataTable({
        "bSort": false,
        "scrollY": yis,
        "scrollX": xis,
        "bLengthChange": false,
        "bFilter": false,
        "keys": true,
        'scrollCollapse': true,
        "bInfo": false,
        'dom': 'Bfrtip',
        "responsive": true,
        'buttons': list,

    });  
};






// Code goes here


/*function zoomPage() {
    var $square = $('.cardbody');

    var viewportHeight = $(window).height();
    var squareHeight = $square.height();

    // Zoom out
    if (squareHeight > (viewportHeight * 1.95)) {
        var pageHeight = $(document).height();
        var desiredSquareHeight = viewportHeight * 1.95;
        var desiredPageHeight = (desiredSquareHeight / squareHeight) * pageHeight;
        var zoom = (desiredPageHeight / pageHeight);
    }
    // Reset the zoom
    else {
        var zoom = 1;
    }

    $('body').css('zoom', zoom);
    $('body').css('-moz-transform', 'scale(' + zoom + ')');
    $('body').css(  '-o-transform', 'scale(' + zoom + ')');
}

// When the browser is resized
$(window).on('resize', function(){ zoomPage(); });

// When the page first loads
$(document).ready(function(){
    zoomPage();
});*/


